...
## OPTIMIZATIONS: Vectorized atoms + Progress bars + Parallel execution + INSTANT RESULTS DISPLAY
#################
"""
...
DATASETS              = ["wine", "breast_cancer"] # ["wine", "breast_cancer", "digits"]
TEST_SIZE             = 0.20
STANDARDIZE           = True
...
# Base atom params
QUANTILES             = (0.15, 0.25, 0.5, 0.75,0.85) #(0.25, 0.5, 0.75)  
ATOM_BETA             = 30.0

# Worldlets
WORLDLETS_PER_CLASS   = 5
...
class DistanceAtomStrategy(BaseAtomStrategy):
    ...
    def generate_atoms_metadata(self, feature_names: List[str], train_data: np.ndarray) -> List[AtomMeta]:
        ...
        atoms = []
        num_prototypes = self.n_prototypes_per_class * self.num_classes  # ✅ Dynamic!
    ...
    def initialize_parameters(self, train_data: np.ndarray, device: str, train_labels: Optional[np.ndarray] = None) -> Dict[str, torch.nn.Parameter]:
        ...
        if train_labels is not None:
            ...
            num_samples = min(num_prototypes, len(train_data))  # ✅ Safe: don't exceed dataset size
...
class MentalModelLayerMultiStrategy(nn.Module):
    ...
    def __init__(self, d_features: int, atom_strategies: List[BaseAtomStrategy],
        ...
        for strategy in atom_strategies:
            # ✅ FIX: Set num_classes for DistanceAtomStrategy before generating atoms
...
# ============================================================
# GRID SEARCH WITH INSTANT RESULTS (UPDATED)
# ============================================================
        ...
        if PARALLEL_JOBS != 1 and HAS_JOBLIB:
            ...
            for row in results:
                if row is not None:
                    ...
                    display_result_row(row)  # 🆕 Print immediately
                    save_result_incrementally(row, csv_path)  # 🆕 Save immediately
        else:
...
if __name__ == "__main__":
    ...
    print("\n[DONE] ✅ Optimized multi-strategy framework complete!")
    print("="*90)